 ...
 *
 * ralph-auth signs all outbound webhook payloads with HMAC-SHA256 using the
 * endpoint's secret. The signature is sent in the `X-Ralph-Auth-Signature`
 * header as `sha256=<hex>`.
 *
 * Use this helper in your webhook receiver to verify that payloads genuinely
 * came from ralph-auth and have not been tampered with.
 *
 * @example
 * ```ts
 * // Next.js App Router route handler
 * import { verifyWebhookSignature } from "@ralph-auth/react/webhook";
 *
 * export async function POST(req: Request) {
 *   const rawBody = await req.text();
 *   const signature = req.headers.get("x-ralph-auth-signature") ?? "";
 *   const secret = process.env.RALPH_AUTH_WEBHOOK_SECRET!;
 *
 ...
 * import { verifyWebhookSignature } from "@ralph-auth/react";
 *
 * app.post("/webhooks/ralph-auth", express.raw({ type: "application/json" }), (req, res) => {
 *   const rawBody = req.body.toString("utf-8");
 *   const signature = req.headers["x-ralph-auth-signature"] ?? "";
 *   if (!verifyWebhookSignature(rawBody, signature, process.env.RALPH_AUTH_WEBHOOK_SECRET!)) {
...
// ── Signature format ─────────────────────────────────────────────────────────
//
// Header:  X-Ralph-Auth-Signature: sha256=<hex>
...
//   X-Ralph-Auth-Signature: t=<unix_ms>,sha256=<hex>
//
...
export interface VerifyOptions {
   ...
   * When set, the `X-Ralph-Auth-Signature` header must include a `t=<timestamp>`
 ...
 * what ralph-auth would have generated with the given secret.
 *
 ...
 * @param signature - The `X-Ralph-Auth-Signature` header value
